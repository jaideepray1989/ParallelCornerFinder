...
import cornerfinders.impl.combination.PerpendicularBisector;
import Jama.Matrix;
...
public class MSEObjectiveFunction implements IObjectiveFunction {
	 ...
	 *         that array's index
	 */
	private double[] calcPathLengths(TStroke stroke) {

		double[] pathLengths = new double[stroke.numPoints()];
		pathLengths[0] = 0.0;

		for (int i = 1; i < stroke.numPoints(); i++) {
	 ...
	 */
	private boolean isLine(int p1, int p2, TStroke stroke,
		...
		TPoint pt1 = stroke.getPoint(p1);
		TPoint pt2 = stroke.getPoint(p2);
	 ...
	 */
	private Matrix curveFitParameters(List<TPoint> points, int k)
			throws java.lang.RuntimeException {
	 ...
	 * @return Error value
	 */
	private double curveFitError(List<TPoint> points, int k) {
	 ...
	 */
	private double arcOrthogonalDistanceSquared(List<TPoint> pts) {

		double error = 0.0;

		TPoint p1 = pts.get(0);
		TPoint p2 = pts.get(pts.size() - 1);
		...
		TPoint midPoint = new TPoint(perpBisector.getMidPoint().getX(),
